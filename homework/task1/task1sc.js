...
let newFullName = fullName.slice(6,14) + fullName.slice(0,6)+ fullName.slice(14,21);
...
console.log([...new Set(arr.filter((num, index, self) => self.indexOf(num) !== index))].sort((a, b) => a - b));
...
let dordunIndexleri = arr.reduce((indices, num, index) => {
    if (num === 4) indices.push(index);
    return indices;
},[]);
    console.log(dordunIndexleri);
...
console.log(arr.filter(num => num > 2), arr.length-arr.filter(num => num > 2).length);
//14) 7 reqeminin indexleri cemini tapin.
let yeddininIndexleri = arr.reduce((indices, num, index) => {
    if (num === 7) indices.push(index);
    return indices;
},[]);
let yeddininIndexlerininCemi = yeddininIndexleri.reduce((sum, index) => sum + index);
...
function enUzunSozunKeyi(arr2) {
    const enUzunSoz = arr2.reduce((longest, obj) => obj.name.length > longest.name.length ? obj : longest);
...
function enUzunSozunIndexininKvadrati(arr2) {
    const enUzunSozunIndexi = arr2.findIndex(obj => obj.name.length === Math.max(...arr2.map(o => o.name.length)));
